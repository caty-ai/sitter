...
ファイルやローテーション後に作られたファイルなら `0`。現在の中身から数え直してはいけません）
と同じであることを確認します。数が増えていれば、まだ何かがそこへ ask を書いています —
超過分の行は取り残された ask なので、まず ask 台帳**側**のロックの下で ask 台帳へ追記し、一度 sweep してから —
そしてローテーションしてはいけません。`sitter run` は
