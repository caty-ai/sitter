...
`ack` を止めません — `"expect_id"` を含む行を `umask 077` 下で順序のまま
**コピー**し、参照先を切り替えてから、その間に古いファイルへ何も入っていないことを
確認する。古いファイルを移動・上書きしない）は
...
ラッパー・スケジューラの登録・ダッシュボードのどれもそのパスを
`expect` / `ack` / `ask` / `watch` / `sweep` に渡していないこと、そして
`grep -c '"expect_id"'` の値が ask を移設した時点で記録した数（ローテーション後に
作られたファイルなら `0`）と同じであることを確認します。数が増えていれば、まだ
何かがそこへ ask を書いているので、ローテーションしてはいけません。`sitter run` は
