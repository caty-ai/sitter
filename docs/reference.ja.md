...
v0.5.0 以降、台帳の行は全プラットフォームでバイト単位で再生され、無効な UTF-8 バイトを含む行も無効扱いになりません（以前はプラットフォーム依存）。出力するテキストは、`iconv` の有無に関わらず有効な UTF-8 の最長の先頭部分に切り詰められます（v0.5.2 以降。それ以前は `iconv` 無しのフォールバックが末尾の不完全なシーケンスしか取り除きませんでした）。
