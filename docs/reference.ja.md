...
既定のログ（`$SITTER_HOME/logs/${RUN_ID}.log`）は一回の実行の出力だけを含みますが、固定の `--log` パスは実行をまたいで増え続けるため運用者が実行と実行の間にローテーションする責任を持ち、子プロセスはログのファイル記述子を開いたままで、実行中のローテーションは出力先とストール時計に使うパスの mtime を分離したり出力を失わせたりするため、実行中には行わないでください。
...
v0.5.0 以降、台帳の行は全プラットフォームでバイト単位で再生され、無効な UTF-8 バイトを含む行も無効扱いになりません（以前はプラットフォーム依存）。出力するテキストは従来どおり、有効な UTF-8 に切り詰めます。
...
sweep は直列化され、期限を迎えた expectation ごとにフックを最大 `SITTER_HOOK_TIMEOUT`（既定 30 秒）待つ間もロックを保持するため、ハングしたフックは フック終了・回収の猶予時間と sweep 自体の処理時間に加え、対象 expectation ごとにその待ち時間だけ次の sweep が処理できる機会を遅らせます。
