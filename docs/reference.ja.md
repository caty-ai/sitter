...
台帳へ移す一度きりの手順（sweep が走れない状態で `"expect_id"` を含む行を
順序のまま**コピー**する。古いファイルを移動・上書きしない）は
[docs/specs/ledger-separation.md](specs/ledger-separation.md)（英語）にあります。

run ファミリーの行しか持たない台帳は、**その台帳の所有者がローテーション**
できます。ローテーションは *rename + 新規ファイル* です。`sitter run` は
...
の間は `<ledger>.lock` を（`flock`、macOS では `lockf -k` で）保持し、
ローテーションを綺麗な境界にしてください。ロックファイルと `<ledger>.lock.d`
ディレクトリには触らないでください。**expect ファミリーの行を含む台帳は
決してローテーションしないでください**: expect ファミリーは履歴全体を再生する
ため、短くなったファイルは置換とみなされて最初から再生され、アクティブな
expectation・準備済みの ask・quarantine の墓標がすべて静かに消えます。その場で
置き換える操作（切り詰め・書き換え・バックアップの上書き復元）は、すべての
台帳で引き続き契約外です。ローテーション済みのファイルは所有者のものです。
sitter はそれを読みません。
