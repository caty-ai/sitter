...
台帳全体を再生します。プロセスの探索だけでは不十分です）。`"expect_id"` を含む行を **新規または空の**ファイルへ `umask 077` と
`<ledger>.lock` の下で順序のまま**コピー**し、同じロックの下で「投影と新ファイルが
バイト単位で一致する」ことを確かめてから参照先を切り替え、再開の直前にもう一度
確かめて、件数を古い台帳の隣に記録する。古いファイルを移動・上書きしない）は
...
ファイルやローテーション後に作られたファイルに限り `0`。現在の中身から数え直してはいけません。
**expect 行を含むファイルに記録が無いのは「不明」であり、記録を復元するまでローテーションも
救出も止めます**。ローテーション後は記録を `0` に戻します）と同じであることを確認します。記録より数が増えていれば、まだ何かがそこへ ask を
書いています — 超過分の行は取り残された ask なので、まず ask 台帳にまだ無いもの（`event_id` で判別）を
ask 台帳**側**のロックの下で（同じロック保持の中で、その id が既に使われている行は拒否して）追記し、追記が成功した場合にだけ「行を取り出したスナップショットの行数」（生きたファイルの数え直しではなく）を記録し、一度 sweep してから — そして
ローテーションしてはいけません。実行できる手順の例は仕様書 §3 B2（英語）にあります。`sitter run` は
...
層 — 同じ方法で取り、保持中なら待つ。ロック無しで rename してはいけません。
参照の監査で見落とした書き手の ask がアーカイブ側に落ちます）で保持し、
ローテーションを綺麗な境界にしてください。それ以外ではロックファイルと
`<ledger>.lock.d` ディレクトリに触らないでください。古いパスをまだ何かが sweep
するなら、新しい expect 台帳には専用の `$SITTER_HOME` を与えてください（sweep の
ロックは home ごとで非ブロッキングなので、同じ home の 2 つの台帳は互いのパスを
飛ばします）。**expect ファミリーが読む台帳は決してローテーションしないでください**:
expect ファミリーはそのパスにあるファイルから履歴全体を再生するため、次のパスは
expectation を一つも含まない新しいファイルをステージし、アクティブな
