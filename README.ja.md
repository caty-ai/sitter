...
<p align="center"><sub>generation: <code>1841769</code> (2026-09-07T06:16:51Z) · verify: <a href="https://api.github.com/repos/caty-ai/sitter/commits/main">API HEAD</a> · <a href="./status.json">status.json</a></sub></p>
<!-- repo-state:end -->
...
![35秒のターミナルデモ。シーン1: sitter に渡したジョブが普通に完走し、アラートファイルは空のまま。シーン2: 2本目のジョブが途中で沈黙すると、sitter が凍結を検知して停止し、台帳には成功と失敗の両方の全イベントが並んで残る。](assets/readme/demo.gif)

[デモをもっと見る](docs/engineering.ja.md#demos) — 凍結検知の詳細版・追加ツールなし版・返事の催促デモ。
...
npm install -g @caty-ai/sitter でインストールして、使い方を教えて
（npm がなければ README の「自分で入れる」の手順で）
```

コマンドまで書いて渡すのは、エージェントに独自の入れ方を発明させないためです。入るのは来歴証明（provenance）付きで公開している公式 npm パッケージ 1 個だけで、`npm update -g @caty-ai/sitter` で最新に保てます。うまくいかないときは、下の手順を自分で実行してください。
...
- **成熟度** — v0.5.0。v0 の 4 つの操作（`run` / `expect` / `ack` / `sweep`）は `docs/requirements-v0.md` で仕様が凍結され、`ask` / `watch` は `docs/specs/prd-v0.2-ask-watch.md` で固定されています（v0.2.0 で監査済み）
